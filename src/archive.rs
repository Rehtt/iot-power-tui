...
pub struct Query { pub from: Option<String>, pub to: Option<String>, pub points: Option<usize> }
impl Query {
    pub fn validate(&self) -> Result<()> {
        ensure!((1..=2000).contains(&self.points.unwrap_or(600)), "points must be 1..2000");
        let a = self.from.as_deref().map(time).transpose()?;
        let b = self.to.as_deref().map(time).transpose()?;
        ensure!(!matches!((a,b), (Some(a),Some(b)) if a>b), "from must not exceed to");
        Ok(())
    }
}
fn time(s: &str) -> Result<i64> { Ok(chrono::DateTime::parse_from_rfc3339(s).context("invalid RFC3339 time")?.timestamp_micros()) }
...
pub fn read(path: &Path, id: i64, query: &Query, cancel: &AtomicBool) -> Result<Response> {
    ...
    ensure!(conn.query_row("SELECT count(*) FROM sessions WHERE id=?1", [id], |r| r.get::<_,i64>(0))? == 1, "session not found");
    let names = columns(&conn,"measurements")?;
    let field = |name: &str, fallback: &str| if names.iter().any(|s|s==name) {format!("coalesce({name},{fallback})")} else {fallback.to_string()};
    ...
    while let Some(row) = rows.next()? {
        ...
        let index = (((t-start) as i128 * capacity as i128 / (span as i128+1)) as usize).min(capacity-1);
        ...
        ensure!(values.iter().chain(min.iter()).chain(max.iter()).all(|x| x.is_finite()), "invalid historical measurement");
        if let Some(p) = buckets.get_mut(&index) {
            let total = p.samples+count;
            let old = [p.voltage_v,p.current_a,p.power_w];
            let means: [f64;3] = std::array::from_fn(|i| old[i]+(values[i]-old[i])*count as f64/total as f64);
            p.voltage_v=means[0];p.current_a=means[1];p.power_w=means[2];p.samples=total;
            for i in 0..3 {p.min[i]=p.min[i].min(min[i]);p.max[i]=p.max[i].max(max[i]);}
            p.break_before |= gap;
...
mod tests {
    ...
    fn full_session_is_weighted_bounded_and_read_only() {
        ...
        let result = read(&path,1,&Query{points:Some(1),..Default::default()},&AtomicBool::new(false)).unwrap();
...
pub fn render(f: &mut ratatui::Frame<'_>, response: &Response, metric: usize) {
    ...
    let mut low = vec![];
    let mut high = vec![];
    let origin = response.from.as_deref().and_then(|s|time(s).ok()).unwrap_or(0);
    let end = response.to.as_deref().and_then(|s|time(s).ok()).unwrap_or(origin+1);
    let mut bounds = [f64::INFINITY,f64::NEG_INFINITY];
    for p in &response.points {
        let x = (time(&p.ts).unwrap_or(origin)-origin) as f64/1e6;
        if p.break_before && !groups.last().unwrap().is_empty() {groups.push(vec![]);}
        let value = [p.voltage_v,p.current_a,p.power_w][metric];
        groups.last_mut().unwrap().push((x,value));
        low.push((x,p.min[metric])); high.push((x,p.max[metric]));
        bounds[0]=bounds[0].min(p.min[metric]); bounds[1]=bounds[1].max(p.max[metric]);
    }
    if !bounds[0].is_finite() {bounds=[0.,1.];}
    let pad = ((bounds[1]-bounds[0])*0.05).max(bounds[0].abs()*0.01).max(1e-9);
    bounds=[bounds[0]-pad,bounds[1]+pad];
    let mut sets: Vec<_> = groups.iter().map(|g|Dataset::default().data(g).graph_type(GraphType::Line).marker(Marker::Braille).style(Color::Yellow)).collect();
...
pub fn browse(paths: Vec<std::path::PathBuf>) -> Result<()> {
    ...
    use ratatui::{backend::CrosstermBackend, widgets::{Paragraph,Block,Borders},Terminal};
    ...
    loop {
        if task.as_ref().is_some_and(|h|h.is_finished()) {
            match task.take().unwrap().join() {
                Ok(Ok(value)) => {response=Some(value);notice.clear();}
        ...
        match key.code {
            ...
            KeyCode::Esc => {
                ...
                if response.take().is_none() {break;}
            }
            KeyCode::Char('q') => {cancel.store(true,Ordering::Relaxed);break;}
            ...
            KeyCode::Enter | KeyCode::Char('r') if task.is_none() => {
                if let Some((path,id,_,_))=rows.get(selection) {
                    ...
                    let token=cancel.clone();
                    notice="读取历史中…".into();
                    task=Some(std::thread::spawn(move ||read(&path,id,&Query::default(),&token)));
                }
            }
            _=>{}
        }
    ...
    cancel.store(true,Ordering::Relaxed);
    if let Some(h)=task {let _=h.join();}
    Ok(())
}

pub fn navigate(response: &Response, key: crossterm::event::KeyCode) -> Option<Query> {
    use crossterm::event::KeyCode;
    if key == KeyCode::Home {return Some(Query::default());}
    let (mut a,mut b)=(time(response.from.as_deref()?).ok()?,time(response.to.as_deref()?).ok()?);
    let span=(b-a).max(2);
    match key {
        KeyCode::Char('[')=>{a+=span/4;b-=span/4;}
        KeyCode::Char(']')=>{a=a.saturating_sub(span/2);b=b.saturating_add(span/2);}
        KeyCode::Left=>{a=a.saturating_sub(span/2);b=b.saturating_sub(span/2);}
        KeyCode::Right=>{a=a.saturating_add(span/2);b=b.saturating_add(span/2);}
        KeyCode::Char('r')=>{}
        _=>return None,
    }
    Some(Query{from:Some(chrono::DateTime::from_timestamp_micros(a)?.to_rfc3339()),to:Some(chrono::DateTime::from_timestamp_micros(b)?.to_rfc3339()),points:Some(600)})
}
