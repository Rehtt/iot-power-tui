...
服务端**直接写入 `--db`**（默认 `./data/iot-power.db`），不使用退出暂存确认。详细样本留在服务端；新版客户端每约 200 ms 拉取统计与最多 60 秒图表桶（优先使用紧凑传输，兼容旧 JSON 服务），不拉取整个数据库。支持多个查看与配置客户端，网络断开时保留最后图表并显示离线，恢复后自动刷新。
...
下载包含选定会话、校准、原始帧及记录数据；下载活动会话前会建立输入屏障，截断当前聚合区间并排空缓存。屏障之后的数据继续采集，但不进入本次下载。进行中的会话下载为一致性部分快照，`export_metadata.partial=1`，不会停止采集或将会话标为完成。文件先写 `.part`，校验后生成唯一 `.db`，不覆盖旧文件；客户端退出会取消下载并清理未完成文件。服务端同时只允许一个导出/下载，忙时可稍后重试。

USB 故障后服务端结束旧会话，按 2、4、8、16、30 秒退避重连，重新读取校准并创建新会话。写入故障停止采集并保留内存缓冲，修复后可在设置窗口重新应用配置以重试排空；回放结束后 HTTP 仍可查询。Ctrl+C / SIGTERM 会排空已接受数据并关闭服务。历史会话不会自动删除。

本期用于**可信局域网**，不带认证、TLS 或设备输出控制。`--addr` 接受 IP:端口（IPv6 使用 `[::1]:8080`）；仅用于 service/client。客户端不接受 `--db` 或本地数据源。API 字段和下载语义见 [Service API](docs/service-api.md)。
...
| `c` | 编辑缓存容量和软件记录速率；Tab 切换字段，Enter 应用，Esc 取消 |
...
本地 TUI 采集期间，达到容量阈值的数据写入目标数据库父目录下的 `.iot-power-pending/capture-随机标识/capture.db`。“已暂存记录”表示已提交临时 SQLite，不表示已写入最终数据库。异常退出遗留的缓存不会自动删除；当前尚无缓存恢复浏览器，请保留提示中的路径。
...
查询完整行推荐 `SELECT * FROM measurement_details`，它会补全从会话继承的设备标识与时间戳状态。原始 USB 帧只存储一次，样本通过 `frame_id` 和 `sample_index` 关联。保存时自动迁移旧 MVP 数据结构，不删除旧测量数据。

CC 每包包含 800 个样本，设备输入固定为 10 kHz。采集解码、实时处理、写库和界面分别运行；完整输入更新数值、能量和图表，不等待 SQLite 提交。图表历史仍最多 60 秒。

### 缓存容量与软件记录速率

```bash
./iot-power-tui --usb --buffer-size 10M --sample-rate 10000
./iot-power-tui --service --buffer-size 10M --sample-rate 333
```

默认每份缓存 **10,000,000 字节**，两份约 20 MB，此外还有有界接收队列、图表历史和数据库开销；并非整个进程的内存上限。支持 `K` / `M`（十进制）、`KiB` / `MiB`（二进制），每份范围 64K–256M。占用按待写结构体及动态容量计数，完整数据块可产生有界超出。接收队列最多 8 个消息。

**没有定时提交**：缓存满后移交写库线程，另一份继续接收；每次事务最多 32 个数据块，SQL 最多 64 行。停止、退出、应用设置和下载活动会话会排空不足阈值的数据。界面分别显示缓存容量、待写记录、已提交记录和正在写盘字节。两份缓存都满时停止采集并报积压；失败缓冲保留，重试仅写未提交部分。SQLite 保持 WAL 和原有同步持久性设置；检查点移到会话结束阶段，运行时须保留 `.db-wal`，下载独立快照请用 API。

`--sample-rate` 是 **1–10000 Hz 软件记录速率**，不是设备采样指令。默认逐样本存储；降速时按测量时间区间记录电压、电流、功率均值和极值、原始样本数及首尾帧引用。功率直接对原始功率求均值，能量始终来自完整输入积分。保留全部 USB 原始帧，因此降速后仍有原始帧存储成本。JSONL 输入不会上采样。

Client 与本地 TUI 均按 `c` 打开设置，窗口打开期间继续采集。应用在后台停止并排空旧会话，再开启新会话；有短暂停采。相同配置不重启正常采集，多个客户端用版本号防止覆盖。配置仅影响本次进程，重启时恢复启动参数。详见 [记录与存储](docs/recording-storage.md)。
...
python3 tests/service_config.py  # 设置、排空失败重试、配置冲突与聚合下载
```
...
| `src/runtime.rs`、`src/recording.rs` | 工作线程、双缓冲、软件降采样与配置校验 |
...
项目处于 MVP 阶段。Linux x86_64/ARM64 与 macOS Intel/Apple Silicon 已通过 v0.0.1 云端构建。USB 实机验证覆盖 Linux x86_64，以及单独构建的树莓派 Zero W ARMv6 静态版本（见[性能验证记录](docs/zero-w-validation.md)）；ARMv6 版本尚未加入 Release 工作流。新增的 i686/ARMv7 目标尚待云端构建及实机验证；Windows 尚未验证。
